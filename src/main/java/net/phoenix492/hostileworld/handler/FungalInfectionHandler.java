...
import net.phoenix492.hostileworld.event.FungalInfectionApplyEffectEvent;
import net.phoenix492.hostileworld.event.FungalInfectionBlockBuildupEvent;
...
public class FungalInfectionHandler {
    ...
    private static void fungalInfectionDropoff(LivingEntity entity) {
        ...
        if (entity.getData(ModDataAttachments.FUNGAL_INFECTION).getInfectionLevel() > HostileWorldConfig.SERVER_CONFIG.fungalInfectionMinimum.get()) {
            entity.getData(ModDataAttachments.FUNGAL_INFECTION).tickScaledReduceInfectionLevel(HostileWorldConfig.SERVER_CONFIG.fungalInfectionUniversalDropoff.get());
        }
    ...
    private static void fungalInfectionApplyEffect(LivingEntity entity) {
        ...
        MobEffectInstance effect = new MobEffectInstance(
            ...
            20 + HostileWorldConfig.SERVER_CONFIG.fungalInfectionPlayerTickFrequency.get(), // Length in ticks equal to how often we check this
            amplifier,
            true,
            true,
            true
    ...
    public static void fungalInfectionRateLimiter(FungalInfectionTickEvent.Pre event) {
        ...
        if (entity instanceof ServerPlayer && level.getGameTime() % HostileWorldConfig.SERVER_CONFIG.fungalInfectionPlayerTickFrequency.get() != 0) {
            event.setCanceled(true);
        }
        else if (level.getGameTime() % HostileWorldConfig.SERVER_CONFIG.fungalInfectionEntityTickFrequency.get() != 0) {
            event.setCanceled(true);
        }
    }
